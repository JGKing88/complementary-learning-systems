...
#SBATCH --mem=16G
#SBATCH --output=slurm_hnav_2phase_%j.out
...
# Two-phase curriculum: load pre-stored continuous checkpoint,
# fine-tune with explore/exploit split.
# Explore phase: agent can store. Exploit phase: Hopfield frozen.

for EXPLORE in 64 32; do
    echo "=== explore_steps=${EXPLORE} / total=128 ==="
    python -m hopfield_nav.train \
        ...
        --eval_every 50 \
        --save_every 500 \
        --save_dir checkpoints/twophase_e${EXPLORE} \
        --seed 42 \
        --device cpu
    echo ""
done
