...
# Probe jobs and verdict jobs write to different log names; take whichever
# exists rather than making the caller remember which kind it launched.
D=/orcd/pool/003/jackking/cls_runs/logs
L=$D/nav_tri_probec_${1}.out
[ -f "$L" ] || L=$D/nav_tri_verdict_${1}.out
[ -f "$L" ] || L=$D/nav_tri_probe_${1}.out
N=${2:-1}
while true; do
    if [ -f "$L" ]; then
