...
_bool action_squash                   "${ACTION_SQUASH:-}"
_bool state_dependent_std             "${STATE_DEPENDENT_STD:-}"
_arg  log_std_min                     "${LOG_STD_MIN:-}"
_arg  log_std_max                     "${LOG_STD_MAX:-}"
_bool action_polar                    "${ACTION_POLAR:-}"
_arg  init_log_kappa                  "${INIT_LOG_KAPPA:-}"
_arg  log_kappa_min                   "${LOG_KAPPA_MIN:-}"
_arg  log_kappa_max                   "${LOG_KAPPA_MAX:-}"
_bool input_abs_position              "${INPUT_ABS_POSITION:-}"
_bool input_chart_frac                "${INPUT_CHART_FRAC:-}"
_bool input_visited                   "${INPUT_VISITED:-}"
_arg  aux_visited_weight              "${AUX_VISITED_WEIGHT:-}"
_arg  aux_visited_radius              "${AUX_VISITED_RADIUS:-}"
_arg  log_kappa_max_end               "${LOG_KAPPA_MAX_END:-}"
_arg  log_kappa_anneal_updates       "${LOG_KAPPA_ANNEAL_UPDATES:-}"
_arg  init_speed_mu                   "${INIT_SPEED_MU:-}"
_arg  init_speed_nu                   "${INIT_SPEED_NU:-}"
_arg  speed_nu_min                    "${SPEED_NU_MIN:-}"
_arg  speed_nu_max                    "${SPEED_NU_MAX:-}"
_arg  speed_mu_eps                    "${SPEED_MU_EPS:-}"
_arg  dir_soft                        "${DIR_SOFT:-}"
_arg  freeze_speed                    "${FREEZE_SPEED:-}"
...
_arg  lr_warmup_updates               "${LR_WARMUP_UPDATES:-}"
_arg  lr_warmup_start_frac            "${LR_WARMUP_START_FRAC:-}"
_arg  adaptive_kl                     "${ADAPTIVE_KL:-}"
_arg  adaptive_kl_factor              "${ADAPTIVE_KL_FACTOR:-}"
_arg  adaptive_lr_min                 "${ADAPTIVE_LR_MIN:-}"
_arg  adaptive_lr_max                 "${ADAPTIVE_LR_MAX:-}"
_arg  gamma                           "${GAMMA:-}"
...
_bool task_novelty_after_store        "${TASK_NOVELTY_AFTER_STORE:-}"
...
_bool reset_kappa_head                "${RESET_KAPPA_HEAD:-}"
_bool eval_deterministic              "${EVAL_DETERMINISTIC:-}"
