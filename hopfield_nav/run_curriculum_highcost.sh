...
#SBATCH --mem=16G
#SBATCH --output=slurm_hnav_highcost_%j.out
...
# Load existing continuous pre-stored checkpoint, fine-tune with store
# Sweep store_cost: 0.1, 0.2, 0.5

for COST in 0.1 0.2 0.5; do
    echo "=== store_cost=${COST} ==="
    python -m hopfield_nav.train \
        ...
        --eval_every 50 \
        --save_every 500 \
        --save_dir checkpoints/curriculum_cont_cost${COST} \
        --seed 42 \
        --device cpu
    echo ""
done
