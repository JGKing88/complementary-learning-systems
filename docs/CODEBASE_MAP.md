...
| `hopfield_nav/` | package | live | Layered since phase 6: `world/` (env, vec_env, scaffold, memory, episode), `policy/`, `rollout/`, `updates/`, `continual/`, `evaluation/`, `training/`, plus the six CLIs at the top level. |
| `hopfield_nav/continual/` | package | live | The continual-learning methods (ER, CLEAR, DER++, online EWC, SI, LwF) behind one `ContinualMethod` interface, added 2026-08-30 for the control suite. Layer 4, alongside `updates/`: a method needs `rollout` and `policy` and nothing above, and `bc_rnn_update` takes plain callables so it never imports one — `training/rnn_sequential.py` composes the two. Not to be confused with `analysis/continual/`, which is the figure pipeline for the same experiment. See [CONTINUAL_CONTROLS_PLAN.md](CONTINUAL_CONTROLS_PLAN.md). |
...
| `docs/` | docs | live | `coordinate_conventions.md`, `sensory_code.md` (what the agent sees, what is settled about it, and which plausible claims about it were measured and turned out wrong), these three files, `REFACTOR_STATUS.md`, and `archive/` (the nine `hopfield_nav/*.md` experiment logs). |
