...
| **FIXED GOALS (§8, 2026-09-15/16)** | Jack: three envs with no goal refresh, then one; "maybe a smaller network". **Three arenas, fixed goals → generalizes** (`fix3_h128`: `follow_q` 0.83/0.65 on unseen arenas, 12.0/14.2 steps; three pairs suffice to make a `q`-follower even at h1024, the small trunk holds it under distractors). **One arena, one fixed goal → generalizes when the goal is interior**, and with h128 it is d0_base-class: `fix1_h128` s43 u3000 (384k episodes) × optimal 1.14/1.14/1.19 vs d0_base's 1.19/1.17/1.37, success 1.0, `follow_q` 0.92–0.98 on every unseen arena, d=0 explore 0.943 vs 0.949; replicated on s44 (1.22/1.21/1.25). Only miss: the d=10 explore tail (18 vs 5). The h1024 control follows `q` too (0.79) but ends less direct (1.31/1.31/1.40). **Wall / near-wall goals become position maps** at every trunk size and exploit-only dropout tried (the 1-D "go to the wall, slide" map is cheaper than atan2(`q`)). Levers: `HIDDEN_SIZE`, `--exploit_obs_dropout`, `--exploit_heading_dropout`, `fix3_*`/`fix1_*` variants; per-arena `follow_q` in `behavior_probe`. Delivered `agent_ckpts/navigate_navp2_fix1_h128_s43_22825505/navigate_u3000.pt`, `..._fix1_h128_s44_22826509/navigate_u3000.pt`, `..._fix3_h128_s42_22783800/navigate_u3300.pt`. Page v8 Part III. |
