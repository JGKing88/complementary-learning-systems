...
class SquareMaze():
    ...
    def get_random_observation(self,):
        self.place_agent_random(direction = True)
        obs = torch.Tensor(self.get_vision()).reshape(1,1000)
        head_direction = one_hot(self.prev_move_global)
        return torch.Tensor([obs, head_direction]), torch.tensor(self.find_optimal_move()).long()
    ...
    def place_agent_random(self, direction = None):
        ...
        if direction: 
            self.prev_move_global = np.random.choice(np.arange(4)) #Random Direction it faces
