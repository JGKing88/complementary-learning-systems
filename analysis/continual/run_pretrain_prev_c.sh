...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_pre_prev_c_%j.out
set -uo pipefail

# =============================================================================
# A pretraining checkpoint WITH the previous-action channel.
#
# Plan decision #1 settled the channel on. No method arm has ever used
# it, because the existing checkpoints were built without it and
# restore_arch_from_ckpt takes input_prev_action from the checkpoint --
# the two imply different input widths, so a loaded checkpoint silently
# overrides the flag. The only way to give the pretrained arms the
# channel is to pretrain with it, which is what this does.
#
# Originally: Blocking for P2/P3/P4 of the discrete
...
#   Gaussian mean+log_std. So `--movement_mode continuous --load_checkpoint
...
OUT_DIR="$CLS_CKPTS_RNN/pretrain_20x20_prev"
LOGS="$REPO/hopfield_nav/logs/wave0d"
mkdir -p "$OUT_DIR" "$LOGS"

echo "[pre-prev-c] repo=$REPO"
echo "[pre-prev-c] out=$OUT_DIR"
echo "[pre-prev-c] started $(date -Is)"
...
python -u -m hopfield_nav.train_rnn \
    ...
    --hidden_size 128 --num_rnn_layers 1 --input_prev_action \
    --lr 1e-3 --epochs 4 --n_minibatches 4 --max_grad_norm 1.0 \
...
STATUS=${PIPESTATUS[0]}
echo "[pre-prev-c] finished $(date -Is)  status=$STATUS"

if [[ -f "$OUT_DIR/final.pt" ]]; then
    echo "[pre-prev-c] checkpoint present: $OUT_DIR/final.pt"
...
a = ck["cfg"]["agent"]
print(f"[pre-prev-c] movement_mode={a['movement_mode']} hidden={a['hidden_size']} "
      f"prev_action={a['input_prev_action']}")
print("[pre-prev-c] head:", [k for k in ck["agent_state_dict"] if "movement" in k])
PY
else
    echo "[pre-prev-c] FATAL: no final.pt written" >&2
    exit 1
fi
