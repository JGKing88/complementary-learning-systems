...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_scratch_%j.out
set -uo pipefail
...
OUT="$CLS_HISTORIES/wave1_fs"
LOGS="$REPO/hopfield_nav/logs/scratch"
mkdir -p "$OUT" "$LOGS"
...
echo "[scratch] repo=$REPO cpus=${SLURM_CPUS_PER_TASK:-$(nproc)} started $(date -Is)"
...
echo "[scratch] launched ${#PIDS[@]} tasks; waiting"
FAILED=()
for k in "${!PIDS[@]}"; do
    if ! wait "${PIDS[$k]}"; then FAILED+=("${NAMES[$k]}"); fi
done
echo "[scratch] finished $(date -Is)"
if (( ${#FAILED[@]} )); then
    echo "[scratch] ${#FAILED[@]} FAILED: ${FAILED[*]:0:20}" >&2
else
    echo "[scratch] all ${#PIDS[@]} tasks OK"
fi
exit 0
