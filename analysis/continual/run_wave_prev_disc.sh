#!/bin/bash -l
#SBATCH --job-name=cl-wave-prev-d
...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_wave_prev_d_%j.out
set -uo pipefail
...
#
# Arms and sweeps are run_scratch_disc.sh's, with --load_checkpoint added back,
...
OUT="$CLS_HISTORIES/wave1d_p"
LOGS="$REPO/hopfield_nav/logs/wave_prev_d"
mkdir -p "$OUT" "$LOGS"
...
CKPT="$CLS_CKPTS_RNN/pretrain_20x20_discrete_prev/final.pt"
if [[ ! -f "$CKPT" ]]; then
    echo "[wave-prev-d] FATAL: checkpoint missing: $CKPT" >&2
    echo "[wave-prev-d]        run analysis/continual/run_pretrain_prev_d.sh first." >&2
    exit 1
fi
...
if [[ "$CK_MODE" != "discrete" || "$CK_PREV" != "True" ]]; then
    echo "[wave-prev-d] FATAL: checkpoint is movement_mode=$CK_MODE prev_action=$CK_PREV;" >&2
    echo "[wave-prev-d]        expected discrete / True." >&2
    exit 1
fi
echo "[wave-prev-d] checkpoint verified: discrete, prev_action on"

BASE=(--load_checkpoint "$CKPT" --lr 1e-3 --epochs 1 --n_minibatches 1 --batch_envs 1)

echo "[wave-prev-d] repo=$REPO cpus=${SLURM_CPUS_PER_TASK:-$(nproc)} started $(date -Is)"
...
echo "[wave-prev-d] launched ${#PIDS[@]} tasks; waiting"
FAILED=()
for k in "${!PIDS[@]}"; do
    if ! wait "${PIDS[$k]}"; then FAILED+=("${NAMES[$k]}"); fi
done
echo "[wave-prev-d] finished $(date -Is)"
if (( ${#FAILED[@]} )); then
    echo "[wave-prev-d] ${#FAILED[@]} FAILED: ${FAILED[*]:0:20}" >&2
else
    echo "[wave-prev-d] all ${#PIDS[@]} tasks OK"
fi
exit 0
