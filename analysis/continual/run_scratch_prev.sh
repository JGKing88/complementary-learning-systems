...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_scratch_p_%j.out
set -uo pipefail
...
LOGS="$REPO/hopfield_nav/logs/scratch_p"
mkdir -p "$OUT" "$LOGS"
...
echo "[scratch-p] repo=$REPO cpus=${SLURM_CPUS_PER_TASK:-$(nproc)} started $(date -Is)"
...
echo "[scratch-p] launched ${#PIDS[@]} tasks; waiting"
FAILED=()
for k in "${!PIDS[@]}"; do
    if ! wait "${PIDS[$k]}"; then FAILED+=("${NAMES[$k]}"); fi
done
echo "[scratch-p] finished $(date -Is)"
if (( ${#FAILED[@]} )); then
    echo "[scratch-p] ${#FAILED[@]} FAILED: ${FAILED[*]:0:20}" >&2
else
    echo "[scratch-p] all ${#PIDS[@]} tasks OK"
fi
exit 0
