...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_scr_prev_d_%j.out
set -uo pipefail
...
LOGS="$REPO/hopfield_nav/logs/scratch_prev_d"
mkdir -p "$OUT" "$LOGS"
...
echo "[scr-prev-d] repo=$REPO cpus=${SLURM_CPUS_PER_TASK:-$(nproc)} started $(date -Is)"
...
echo "[scr-prev-d] launched ${#PIDS[@]} tasks; waiting"
FAILED=()
for k in "${!PIDS[@]}"; do
    if ! wait "${PIDS[$k]}"; then FAILED+=("${NAMES[$k]}"); fi
done
echo "[scr-prev-d] finished $(date -Is)"
if (( ${#FAILED[@]} )); then
    echo "[scr-prev-d] ${#FAILED[@]} FAILED: ${FAILED[*]:0:20}" >&2
else
    echo "[scr-prev-d] all ${#PIDS[@]} tasks OK"
fi
exit 0
