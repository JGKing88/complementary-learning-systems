...
#SBATCH --output=/home/jackking/cls/hopfield_nav/logs/slurm_cl_pre_prev_d_%j.out
set -uo pipefail
...
OUT_DIR="$CLS_CKPTS_RNN/pretrain_20x20_discrete_prev"
LOGS="$REPO/hopfield_nav/logs/wave0d"
mkdir -p "$OUT_DIR" "$LOGS"

echo "[pre-prev-d] repo=$REPO"
echo "[pre-prev-d] out=$OUT_DIR"
echo "[pre-prev-d] started $(date -Is)"
...
STATUS=${PIPESTATUS[0]}
echo "[pre-prev-d] finished $(date -Is)  status=$STATUS"

if [[ -f "$OUT_DIR/final.pt" ]]; then
    echo "[pre-prev-d] checkpoint present: $OUT_DIR/final.pt"
...
a = ck["cfg"]["agent"]
print(f"[pre-prev-d] movement_mode={a['movement_mode']} hidden={a['hidden_size']} "
      f"prev_action={a['input_prev_action']}")
print("[pre-prev-d] head:", [k for k in ck["agent_state_dict"] if "movement" in k])
PY
else
    echo "[pre-prev-d] FATAL: no final.pt written" >&2
    exit 1
fi
