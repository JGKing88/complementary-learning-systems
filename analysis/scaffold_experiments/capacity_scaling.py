...
from analysis.scaffold_experiments.encoder_scaffold import (
    ScaffoldCommon,
    EncoderScaffold,
...
def _make_common(seed: int, n_envs: int, env_size: int, Npos: int, Ns: int,
    ...
    """Thin wrapper so we can reuse `build_common` with a fresh args namespace."""
    ns = types.SimpleNamespace(
        seed=seed, n_envs=n_envs, env_size=env_size, Npos=Npos,
        Ns=Ns, lambdas=lambdas,
    )
    return build_common(ns, lambdas)
