...
complete `encoder_training.nav_eval.run_navigation_eval` for aggregate metrics.
"""
...
# matplotlib is imported inside the plot_* functions below: the simulation half
# of this module is plain numpy/torch, and test_layering keeps the figure stack
# out of library packages' import time. The `plt.Axes` annotations survive
# because of `from __future__ import annotations` above.

from hopfield import Hopfield
from .nav_eval.nav import compute_projection_matrix, continuous_step
from .nav_eval.nav import simulate_trajectory as _cls_simulate_trajectory
...
def projection_matrix_at(encoded_grid: np.ndarray, gx: int, gy: int,
    ...
    """Thin wrapper around `encoder_training.nav_eval.nav.compute_projection_matrix`."""
...
def projected_step(
    ...
    Delegates to `encoder_training.nav_eval.nav.continuous_step` for (dgx, dgy, magnitude),
...
def simulate_trajectory(
    ...
    Delegates the core simulation to `encoder_training.nav_eval.nav.simulate_trajectory`. Also
